...
For any pair genetic location there is a probability model govering the joint distribution of the sources of the inherited alleles. That is, a genotyped final line will have an allele at marker $M_1$ inherited from some founder line, and another allele at marker $M_2$ inherited from a (potentially different) founder line. Note that we ignore the fact that founders may have identical alleles; it is the source of the allele that is important. 
