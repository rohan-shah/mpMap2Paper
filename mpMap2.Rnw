...
The situation appears to be more complicated the larger the number of founders. For the eight-way design with a single funnel and infinite generations of selfing, the following marker allele distribution is \emph{approximately uninformative} for the parameter $r$. <<echo=FALSE>>=
...
Determining whether a pair of markers is informative can in theory be done by testing whether the derivative of the likelihood is always zero
