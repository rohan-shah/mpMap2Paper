...
@
Observed values $1 - 4$ correspond to homozygotes for founder lines, and values $5 - 10$ correspond to different hetrozygotes. We specified $2$ generations of selfing and this is reflected in the distribution of observed values for the final population. 
<<cache=TRUE>>=
table(crossSingleFunnel@geneticData[[1]]@finals[,1])
@

The {\color{red}most common types of markers} (I think?) currently used are \emph{Single Nucleotide Polymorphism} (SNP) markers. To convert our simulated data objects to these types of markers, we combine them with a call to \code{multiparentSNP}. As the object \code{combined} contains multiple data sets, each one must be converted individually. 
...
@
As we originally specified the \code{selfing} slot to have value \code{"auto"}, the imputed values will contain hetrozygotes. The encoding of hetrozygotes is given in an entry named \code{key}. Comparing the imputed data to the original data (before the markers were converted to SNP markers) shows good agreement for the first data set, even for the hetrozygotes. 
<<cache=TRUE>>=
imputed@geneticData[[1]]@imputed@key
table(imputed@geneticData[[1]]@imputed@data, combined@geneticData[[1]]@finals)
@
The pattern of imputation errors for the hetrozygotes makes sense; value $5$ is a hetrozygote of founders $1$ and $2$, and the most frequent imputation error is to classify it as a homozygote of founder $1$ or $2$. 

For the second data set no hetrozygotes are imputed. This is because no hetrozygote markers were called, so hetrozygotes are either missing or consistent with being homozygotes, in which case the homozygote is always more likely. The only clue in the data is that missing values are always hetrozygotes in this case. We can therefore alter the model to include a probability of observing a missing value. Setting \code{hetrozygoteMissingProb} to $1$ and \code{homozygoteMissingProb} to $0.05$ gives acceptable results. 
<<cache=TRUE,tidy=TRUE,tidy.opts=list(width.cutoff=56)>>=
table(imputed@geneticData[[2]]@imputed@data, combined@geneticData[[2]]@finals)
imputed <- imputeFounders(mappedSNP, hetrozygoteMissingProb = 1, homozygoteMissingProb = 0.05)
table(imputed@geneticData[[2]]@imputed@data, combined@geneticData[[2]]@finals)
@
The miss-classifications still demonstrate the same problem to a lesser extent. Value $5$ is a hetrozygote of founders 1 and 2, and just under 50\% of these values are miss-classified as homozygotes of founders $1$ or $2$. 
...
We begin with two examples showing that \pkg{mpMap2} can construct correct maps from unusual experimental designs. The last example involves real data from a four-parent cross. 

\subsection{No intecrossing or selfing}

<<cache=TRUE,tidy.opts=list(width.cutoff=56)>>=
...
map <- qtl::sim.map(len = rep(300, 2), n.mar = 101, anchor.tel = TRUE, include.x = FALSE, eq.spacing = FALSE)
cross <- simulateMPCross(pedigree = pedigree, map = map, mapFunction = haldane, seed = 1)
#Randomly rearrange markers
cross <- subset(cross, markers = sample(markers(cross)))
rf <- estimateRF(cross, verbose = list(verbose = TRUE, progressStyle = 1))
grouped <- formGroups(cross, groups = 3)
